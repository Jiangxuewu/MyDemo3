...
public class ScreenHelper {
    ...
    public void lockScreen(Context context) {
        ...
        if (Build.VERSION.SDK_INT >= 20) {
            CMD.doExec("su root input keyevent " + KeyEvent.KEYCODE_SLEEP);
        } else if(!isSleep(context)){
            CMD.doExec("su root input keyevent " + KeyEvent.KEYCODE_POWER);
        }
    }

    public void unlockScreen(Context context) {
        L.i(TAG, "unlockScreen");
        //按下开屏键
        if (Build.VERSION.SDK_INT >= 20) {
            CMD.doExec("su root input keyevent " + KeyEvent.KEYCODE_WAKEUP);
        } else if(isSleep(context)){
            CMD.doExec("su root input keyevent " + KeyEvent.KEYCODE_POWER);
        }
    }
